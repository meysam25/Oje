...
namespace Oje.Section.WebMain.Services
{
    public class PageSliderService : IPageSliderService
    {
        ...
        {
            var foundItem = db.PageSliders.Where(t => t.SiteSettingId == siteSettingId && t.Id == id).FirstOrDefault();
            if (foundItem == null)
        ...
        public GridResultVM<PageSliderMainGridResultVM> GetList(PageSliderMainGrid searchInput, int? siteSettingId)
        {
            ...
            var quiryResult = db.PageSliders.Where(t => t.SiteSettingId == siteSettingId);
        ...
        public ApiResult Update(PageSliderCreateUpdateVM input, int? siteSettingId, long? loginUserId)
        {
            ...
            var foundItem = db.PageSliders.Where(t => t.SiteSettingId == siteSettingId && t.Id == input.id).FirstOrDefault();
            if (foundItem == null)
                throw BException.GenerateNewException(BMessages.Not_Found);
