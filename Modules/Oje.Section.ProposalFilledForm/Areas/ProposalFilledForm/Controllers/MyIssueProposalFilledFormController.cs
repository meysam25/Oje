...
namespace Oje.Section.ProposalFilledForm.Areas.ProposalFilledForm.Controllers
{
    ...
    [AreaConfig(ModualTitle = "مدیریت بیمه نامه ها", Order = 12,  Icon = "fa-file-powerpoint", Title = "بیمه نامه های من (صادر شده)")]
    [CustomeAuthorizeFilter]
    public class MyIssueProposalFilledFormController: Controller
    {
        readonly List<ProposalFilledFormStatus> validStatus =
            new List<ProposalFilledFormStatus>() { ProposalFilledFormStatus.Issuing };
        ...
        public MyIssueProposalFilledFormController
            (
        ...
        public IActionResult GetJsonConfig()
        {
            ...
            return Content(System.IO.File.ReadAllText(GlobalConfig.GetJsonConfigFile("ProposalFilledForm", "MyIssueProposalFilledForm")));
        }

        [AreaConfig(Title = "دانلود پی دی اف بیمه نامه های من (صادر شده)", Icon = "fa-download")]
        [HttpGet]
        public IActionResult DownloadPdf([FromQuery] GlobalLongId input)
        {
            return File(
                    HtmlToPdfBlink.Convert((Request.IsHttps ? "https" : "http") + "://" + Request.Host + Url.Action("Detaile", "MyIssueProposalFilledForm", new { area = "ProposalFilledForm", id = input.id, isPrint = true }), Request.Cookies),
                    System.Net.Mime.MediaTypeNames.Application.Pdf, DateTime.Now.ToFaDate("_") + "_" + DateTime.Now.ToString("HH_mm_ss") + ".pdf"
                );
        }

        [AreaConfig(Title = "جزییات بیمه نامه های من (صادر شده)", Icon = "fa-eye")]
        [HttpGet]
        ...
        [AreaConfig(Title = "مشاهده اسناد بیمه نامه های من (صادر شده)", Icon = "fa-eye")]
        [HttpPost]
