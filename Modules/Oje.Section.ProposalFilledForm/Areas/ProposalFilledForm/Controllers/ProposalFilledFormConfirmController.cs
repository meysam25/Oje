...
namespace Oje.Section.ProposalFilledForm.Areas.ProposalFilledForm.Controllers
{
    ...
    public class ProposalFilledFormConfirmController : Controller
    {
        ...
        [AreaConfig(Title = "لیست فرم پیشنهاد تایید شده", Icon = "fa-check-circle", IsMainMenuItem = true)]
        [HttpGet]
        ...
        {
            return Json(ProposalFilledFormAdminService.GetList(searchInput, SiteSettingService.GetSiteSetting()?.Id, HttpContext.GetLoginUser()?.UserId, ProposalFilledFormStatus.Confirm));
        }
        ...
        {
            var result = ProposalFilledFormAdminService.GetList(searchInput, SiteSettingService.GetSiteSetting()?.Id, HttpContext.GetLoginUser()?.UserId, ProposalFilledFormStatus.Confirm);
            if (result == null || result.data == null || result.data.Count == 0)
                return NotFound();
            var byteResult = ExportToExcel.Export(result.data);
            if (byteResult == null || byteResult.Length == 0)
