...
namespace Oje.Section.ProposalFilledForm.Areas.ProposalFilledForm.Controllers
{
    ...
    public class ProposalFilledFormNeedSpecialistController : Controller
    {
        ...
        [AreaConfig(Title = "لیست فرم پیشنهاد نیاز به کارشناسی", Icon = "fa-file", IsMainMenuItem = true)]
        [HttpGet]
        ...
        {
            return Json(ProposalFilledFormAdminService.UpdateAgent(id, userId, SiteSettingService.GetSiteSetting()?.Id, HttpContext.GetLoginUser()?.UserId, ProposalFilledFormStatus.NeedSpecialist));
        }
        ...
        public IActionResult PdfDetailesForAdmin([FromQuery] long id, [FromQuery] bool isPrint = false)
        {
            ...
            return View(ProposalFilledFormAdminService.PdfDetailes(id, SiteSettingService.GetSiteSetting()?.Id, HttpContext.GetLoginUser()?.UserId, ProposalFilledFormStatus.NeedSpecialist));
        }
        ...
        {
            return Json(ProposalFilledFormAdminService.GetList(searchInput, SiteSettingService.GetSiteSetting()?.Id, HttpContext.GetLoginUser()?.UserId, ProposalFilledFormStatus.NeedSpecialist));
        }
        ...
        {
            var result = ProposalFilledFormAdminService.GetList(searchInput, SiteSettingService.GetSiteSetting()?.Id, HttpContext.GetLoginUser()?.UserId, ProposalFilledFormStatus.NeedSpecialist);
            if (result == null || result.data == null || result.data.Count == 0)
                return NotFound();
            var byteResult = ExportToExcel.Export(result.data);
            if (byteResult == null || byteResult.Length == 0)
