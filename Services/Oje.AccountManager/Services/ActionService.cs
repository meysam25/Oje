    ...
    public class ActionService: IActionService
    {
        ...
        public object GetightListForSelect2(Select2SearchVM searchInput, int? siteSettingId)
        {
            ...
            if (!string.IsNullOrEmpty(searchInput.search))
                qureResult = qureResult.Where(t => (t.Controller.Section.Title + "/" + t.Title).Contains(searchInput.search));
            qureResult = qureResult.Skip((searchInput.page.Value - 1) * take).Take(take);
            if (qureResult.Count() >= 50)
                hasPagination = true;

            result.AddRange(qureResult.Select(t => new { id = t.Id, text = t.Controller.Section.Title + "/" + t.Title }).ToList());
